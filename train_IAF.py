...
from models.IAF import VAE
from deepul.hw2_helper import *
from arguments import parsing


class Solver(object):
    def __init__(self, learning_rate=1e-5, n_epochs=400, local_rank=None, args=None):
...
class IAFSolver(Solver):
    ...
    def build(self):
        self.model = VAE().cuda()
        self.model = DDP(module=self.model, device_ids=[self.local_rank], find_unused_parameters=True)
...
def main():
    ...
    solver = IAFSolver(n_epochs=args.num_epoch, local_rank=local_rank, args=args)
    solver.build()
    solver.train()
